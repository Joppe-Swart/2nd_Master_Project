...
def plots(depol, imagename, x_lambda, norm, a, curv, p0, X0, RM, sigma, y_i, err_i, y_q, err_q, y_u, err_u, y_X, err_X, y_p, err_p, xlim=[0.005, 0.06], imgformat="pdf",saveplot=True):
  ...
  freqs = np.array([0.9,1.0,1.2,1.5,2.0,3.0])*1.e9
  ...
  ax1.errorbar(x_lambda, 1e3*y_i, yerr=1e3*err_i, fmt='o', color='k', mfc='none', markersize=6,elinewidth=1.)
  ax2.errorbar(x_lambda, 1e3*y_q, yerr=1e3*err_q, fmt='o', color='k', mfc='none',markersize=6,elinewidth=1.)
  ax3.errorbar(x_lambda, 1e3*y_u, yerr=1e3*err_u, fmt='o', color='k', mfc='none',markersize=6,elinewidth=1.)
  ...
  leg1, = ax1.plot(x, 1e3*fit_i,'-',color='royalblue',linewidth=5.,zorder=len(y_i)+1, label=r"$\chi_{\rm red, I}^2=$"+str(round(chisqred_i,2)))
  leg2, = ax2.plot(x, 1e3*fit_q,'-',color='royalblue',linewidth=5.,zorder=len(y_q)+1, label=r"$\chi_{\rm red, QU}^2=$"+str(round(chisqred_qu,2)))
  ax3.plot(x, 1e3*fit_u,'-',color='royalblue',linewidth=5.,zorder=len(y_u)+1)
  ...
  ax1res.errorbar(x_lambda, 1e3*(y_i-model_i), yerr=1e3*err_i,fmt='.',markersize=4,color='k',alpha=0.75)
  ax1res.plot(x,[0]*1000,'--',color='royalblue',linewidth=2.5, zorder=len(y_i)+1)
  ax2res.errorbar(x_lambda, 1e3*(y_q-model_q), yerr=1e3*err_q,fmt='.',markersize=4,color='k',alpha=0.75)
  ax2res.plot(x,[0]*1000,'--',color='royalblue',linewidth=2.5, zorder=len(y_q)+1)
  ax3res.errorbar(x_lambda, 1e3*(y_u-model_u), yerr=1e3*err_u,fmt='.',markersize=4,color='k',alpha=0.75)
