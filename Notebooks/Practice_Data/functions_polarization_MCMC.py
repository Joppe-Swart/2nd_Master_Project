...
def polarization_fitting(wave, fluxI, err_fluxI, fluxQ, err_fluxQ, fluxU
  ...
  def lnprior_pol(theta):
    if depol == "ExtDepol" or depol == "IntDepol" or depol == "CombDepol":
      ...
      if 0.<=p0<=1. and sigma_rm>=0. and (-400 <= rm <= 400):# and -numpy.pi<=chi0<numpy.pi
        return 0.	
    elif depol == 'ExtDepol_RePol': # For ExtDepol_Repol : sigma_RM unconstrained between -inf and +inf
      p0, chi0, rm, sigma_rm = theta
      if 0.<=p0<=1. and 0.<=chi0<numpy.pi and (-400 <= rm <= 400):
        return 0.       
    else: 
      p0, chi0, rm = theta
      if 0.<=p0<=1. and 0.<=chi0<numpy.pi and (-400 <= rm <= 400):
...
######################################################################################################
  ...
  if depol == "":
    ...
    if cornerplot:
      ...
      matplotlib.pyplot.text(0.7,3,"Initial RM guess: %.2f [rad m$^{-2}$]"%rm,transform=fig.axes[6].transAxes)
    ...
    if cornerplot:
      ...
      matplotlib.pyplot.text(0.7,1,"Initial RM guess test: %.2f [rad m$^{-2}$]"%rm,transform=fig.axes[6].transAxes)
    ...
    if cornerplot:
      ...
      matplotlib.pyplot.text(0.7,0.7,"Initial RM guess test: %.2f"%rm)
