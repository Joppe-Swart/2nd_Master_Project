...
def integratedflux(fitsimage, ds9region, i, hdul=None, test=False):
    ...
    head = flatten(hdul).header
    data = hdul[0].data 
    if data.ndim == 4:
        data = data[0,0] # remove Stokes and Freq axis
    w = wcs.WCS(head)

    if type(ds9region) == str or type(ds9region) == np.str_:
    ...
    rpix = r[i].to_pixel(w.celestial)
    rmask = rpix.to_mask()
    masked_data = rmask.cutout(data) # This is the entire block around the source
    if masked_data is None:
        print("WARNING: Region is likely outside of the image, returning NaN")
...
#    print(totalflux.value, Nbeams)
    return totalflux.value, Nbeams # flux given in Jy

def uncertainty_flux(fitsimage, flux, Nbeams, i, regionfile_rms=None, rms=None, delta_cal=0.1, hdul=None
    ,verbose=True):
    """
